...
public class ZAudioUDBSocket {
	
	
	public ZAudioUDBSocket() {
	...
	public boolean audioControllCommand(ZaudioInZone zaudioInZone,byte typeOfControll,byte paramter1,byte paramter2,byte paramter3){
		
		boolean blnSuccess = false, blnSent = false,blnNeedToWaitANS=false,blnNeedResend=true;
		...
		try {
			...
			arrayAddtional[0] =  typeOfControll;
			arrayAddtional[1] = paramter1;
			arrayAddtional[2] = paramter2;
			arrayAddtional[3] = paramter3;
			...
			for (byteI = 1; byteI <= SmartSocketConnection.CONST_MAX_TIMES_OF_SEND; byteI++) {
				...
				blnSent = SendUDPBuffer(arrayAddtional, shortLenOfAddtionalBuf,
						intOP, (byte)zaudioInZone.getSubnetID(), (byte)zaudioInZone.getDeviceID(), false);
				if (blnSent == true) // 如果发送成功
				{
					// 与否只是做一个程序转责的一种标志哦
					if (blnNeedToWaitANS == true)// 在发送成功的条件下如果需要等待回应，就要接受数据报包
					{
						arraybyteBufWithoutAA = UDPReceive((byte)zaudioInZone.getSubnetID(),
								(byte)zaudioInZone.getDeviceID(), intOP, true);
	...
	public boolean audioControllCommand(byte subnetID,byte deveiceID,byte typeOfControll,byte paramter1,byte paramter2,byte paramter3){
		...
		try {
			...
			for (byteI = 1; byteI <= SmartSocketConnection.CONST_MAX_TIMES_OF_SEND; byteI++) {
				...
				blnSent = SendUDPBuffer(arrayAddtional, shortLenOfAddtionalBuf,
						intOP, subnetID, deveiceID, false);
				if (blnSent == true) // 如果发送成功
				{
					// 与否只是做一个程序转责的一种标志哦
					if (blnNeedToWaitANS == true)// 在发送成功的条件下如果需要等待回应，就要接受数据报包
					{
						arraybyteBufWithoutAA = UDPReceive(subnetID,
								deveiceID, intOP, true);
						if (arraybyteBufWithoutAA == null) {
							if ((blnNeedResend == false)
									|| (My_app.mblnNeedCancelToWaitInUDPSocket == true)) {
								return false;
	...
	public byte[] ProcessUDPPackets(byte[] arraybyteRec) {
	 ...
	 public byte[] ReadTotalQTYofBigPackagesforZAudioAlbum(byte byteSubnetID,byte byteDeviceID,byte sourceNo)
	    {
	    	boolean blnSuccess=false;
	    	byte[] arraybyteBufWithoutAA=null;
	    	try
	 ...
	 public byte[] ReadZoneStatus(byte byteSubnetID,byte byteDeviceID)
	    {
	    	boolean blnSuccess=false;
	    	byte[] arraybyteBufWithoutAA=null;
	    	try
	 ...
	 public byte[] ReadTotalQTYofBigPackagesforZAudioSong(byte byteSubnetID,byte byteDeviceID,byte sourceNo,byte albumNo )
	    {
	    	boolean blnSuccess=false;
	    	byte[] arraybyteBufWithoutAA=null;
	    	try
	 ...
	 public byte[] readTotalAlbum(byte byteSubnetID,byte byteDeviceID,byte sourceNo,byte currentBigNo)
	    {
	    	boolean blnSuccess=false;
	    	byte[] arraybyteBufWithoutAA=null;
	    	try
	    	{
	    		...
	    		byte[] arrayAddtional =new byte[2];
	    	    	   arrayAddtional[0]=sourceNo;
	    	    	   arrayAddtional[1]=currentBigNo; // Current Big Package No
	    		shortLenOfAddtionalBuf=(short) (arrayAddtional.length);
	    		blnSuccess=SendUDPBuffer(arrayAddtional, shortLenOfAddtionalBuf, intOP, byteSubnetID, byteDeviceID, false);
	    		if (blnSuccess==true)	
	     	    {
	 ...
	 public byte[] readTotalAlbumSong(byte byteSubnetID,byte byteDeviceID,byte sourceNo,byte albumno,byte pak )
	    {
	    	boolean blnSuccess=false;
	    	byte[] arraybyteBufWithoutAA=null;
	    	try
	    	{
	    		...
	    		byte[] arrayAddtional =new byte[3];
	    	    	   arrayAddtional[0]=sourceNo;
	    	    	   arrayAddtional[1]=albumno;
	    	    	   arrayAddtional[2]=pak; // Current Big Package No
	    		shortLenOfAddtionalBuf=(short) (arrayAddtional.length);
	    		blnSuccess=SendUDPBuffer(arrayAddtional, shortLenOfAddtionalBuf, intOP, byteSubnetID, byteDeviceID, false);
	    		if (blnSuccess==true)	
	     	    {
	    ...
	    public byte[] MusicZonePowerControl(byte byteSubnetID,byte byteDeviceID,byte byteZoneNo,boolean blnPowerOn,boolean blnNeedResend,boolean blnNeedToWaitANS)
	    {
	    	...
	    	try
	    	{
	    		...
	    		for(byteI=1;byteI<=CONST_MAX_TIMES_OF_SEND;byteI++)
	 	    	{
	    		    if (SmartSocketConnection.IsSocketClose()==true) return null;
	    	...
	    	try
	    	{
	    		...
	    		while((lngCurTime_of_MS-lngStartTime_of_MS)<=intTimeout)
	    		{
	    			try
	        		{
	    				...
	    				SmartSocketConnection.moUDPSocket.setSoTimeout(CONST_TIME_OUT_FOR_EACH_WAIT);
	    				SmartSocketConnection.moUDPSocket.receive(oPacketRec);
	    				arraybyteBufRec=oPacketRec.getData();
