...
public class FanUDPSocket {
	...
	public void changFanStatus(FanInZone fanInZone, int gear) {
		byte byteSubnetID, byteDeviceID;

		int intTemp = (Integer) fanInZone.getSubnetID();
		...
		changeFanStatus(byteSubnetID, byteDeviceID, gear,
				fanInZone.getChannelNO(), false, false);

	}

	public boolean changeFanStatus(byte byteSubnetID, byte byteDeviceID,
			int gear, int chanel,
		...
		try {
			...
			arrayAddtional[0] = (byte) chanel;
			arrayAddtional[1] = (byte) gear;
			arrayAddtional[2] = 0;
			arrayAddtional[3] = 0;
