...
public class RGBSocket {
	...
	public boolean rgbtest(int subnet,int device,byte red,byte green,byte blue,byte white){
		...
		try {
			...
			arrayAddtional[0] =  red;  
			arrayAddtional[1] = green;
			arrayAddtional[2] = blue;
			arrayAddtional[3] = white;
			arrayAddtional[4] = 0;
			arrayAddtional[5] = 0;
			...
			for (byteI = 1; byteI <= SmartSocketConnection.CONST_MAX_TIMES_OF_SEND; byteI++) {
				...
				blnSent = SendUDPBuffer(arrayAddtional, shortLenOfAddtionalBuf,
						intOP, (byte)subnet, (byte)device, false);
				if (blnSent == true) 
				{
					if (blnNeedToWaitANS == true)
					{
						arraybyteBufWithoutAA = UDPReceive((byte)subnet,
								(byte)device, intOP, true);
